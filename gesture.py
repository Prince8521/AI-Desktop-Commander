...
while True:
    ...
    if hands:
        for hand in hands:
            ...
            # Check for the quit gesture (showing a small finger)
            thumb_tip_y = None
            index_tip_y = None
            ...
            if thumb_tip_y is not None and index_tip_y is not None and thumb_tip_y < index_tip_y:
                print("Quit gesture detected. Exiting the program.")
                cap.release()
    ...
    if cv2.waitKey(1) & 0xFF == ord('q'):
        print("Quit requested. Exiting the program.")
        cap.release()
        cv2.destroyAllWindows()
        break
